...
Public Function Grava_Moeda(ByVal Valor As Variant) As Variant
    ...
    If Valor = Empty Or Valor = " " Or Valor = "" Or IsNull(Valor) = True Then
        Grava_Moeda = 0
        Exit Function
    End If
    ...
    'INSERIDO PARA CORRIGIR O ERRO DE MAIS DE UMA VIRGULA NO VALOR (RAFAEL)
    If Not IsNumeric(strDecimal) Then strDecimal = "00"
    strDecimal = Format(strDecimal, "####0000")
...
Public Function Verifica_Numero(Nome_Campo As String, Nome_Tabela As String, Nome_textbox As TextBox) As Boolean
    ...
    On Error GoTo Erro
    SQL = ""
    SQL = "SELECT " & Nome_Campo & " FROM " & Nome_Tabela & " " & _
    ...
    If Val(TBrecordset(Nome_Campo)) = Val(Nome_textbox.Text) Then
       MsgBox "Registro já existente.", vbInformation, "Only Tech"
...
Public Function Valida_Trava_Sistema(Form As Form) As Boolean
    ...
    If rstValida_trava!DFBloqueado_TBPedidos_validos = True Then
...
End Function
Public Function Abre_porta_impressora_via_LPT1(Caminho_impressora)

    Open Caminho_impressora For Output As #1  'o caminho lpt1 pode ser alterado para caminho de rede (lpt2,com1,etc)
    
End Function
Public Function Fecha_porta_impressora_via_LPT1()
    Close #1
End Function
Public Function Nome_maquina() As String
    ...
    Nome_maquina = Left$(strString, InStr(1, strString, Chr$(0)) - 1)

End Function
Function Abrir_impressora_registro(Aplicacao As String, Form As Object) As String
      ...
      Caminho = Registro.WinRegLerSequência("HKEY_LOCAL_MACHINE\SOFTWARE\" + Aplicacao + "\PRN", "Caminho")

      Abrir_impressora_registro = Caminho
...
Function Verifica_integracao_portal(cod_empresa As Integer, Form As Object) As Boolean

    Dim rstPortal As New ADODB.Recordset
    Dim strSql_Portal As String

    On Error GoTo Erro

    strSql_Portal = Empty
    strSql_Portal = "SELECT DFIntegra_portal_TBParametros_gerais FROM TBParametros_gerais WHERE PFKCodigo_TBEmpresa = " & cod_empresa & ""

    Movimentacoes.Select_geral strSql_Portal, "BDRetaguarda", rstPortal, "Otica", Form

    If rstPortal.BOF = True And rstPortal.EOF = True Then
       Verifica_integracao_portal = False
       Set rstPortal = Nothing
       Exit Function
    End If

    Verifica_integracao_portal = rstPortal!DFIntegra_portal_TBParametros_gerais
...
Function Verifica_registros_nao_integrados(Tabela_retaguarda As String, Campos_select_retaguarda As String, Aplicacao_retaguarda As String, Campo_verificador_integracao_retaguarda As String, rstDescarregar_registros As ADODB.Recordset, Form As Form) As Boolean

    Dim strIntegracao As String
    Dim rstReg_Integracao As New ADODB.Recordset

    On Error GoTo Erro

    strIntegracao = Empty
    strIntegracao = "SELECT " & Campos_select_retaguarda & " FROM " & Tabela_retaguarda & " WHERE " & Campo_verificador_integracao_retaguarda & " = 0 or " & Campo_verificador_integracao_retaguarda & " IS NULL"
    Movimentacoes.Select_geral strIntegracao, "BDRetaguarda", rstReg_Integracao, Aplicacao_retaguarda, Form

    If rstReg_Integracao.BOF = True And rstReg_Integracao.EOF = True Then
       Verifica_registros_nao_integrados = False
    Else
       Verifica_registros_nao_integrados = True
       'Movendo os registros para a recordset
       Set rstDescarregar_registros = rstReg_Integracao
    End If
    
    Set rstReg_Integracao = Nothing
...
Function Atualiza_registros_nao_integrados(rstReg_nao_atualizados As ADODB.Recordset, Tabela_portal As String, Campo_comparacao_portal As String, Campo_comparacao_retaguarda As String, Aplicacao_Portal As String, Campos_gravar_portal As String, Form As Form, Banco_portal As String, Aplicacao_retaguarda As String, Banco_retaguarda As String, Tabela_retaguarda As String, Campo_Integracao_Retaguarda As String, Optional Usuario As String, Optional Senha As String, Optional Qtd_campos_values_insert_retaguarda As String)

    Dim Sql_Portal As String
    Dim intCont As Integer

    Do While rstReg_nao_atualizados.EOF = False

       Dim rstIncluso_portal As New ADODB.Recordset
       Dim strComparacao_retaguarda_portal As String
       Dim strValues As String

       strComparacao_retaguarda_portal = rstReg_nao_atualizados.Fields(Campo_comparacao_retaguarda)

       'Excluindo informações anteriores que foram excluidas no Retaguarda
       'Objetivo é varrer as tabelas do portal e comparar com as do retaguarda e se for o caso eliminar
       'os registros que só venham a existir no Portal,pois foram excluidos
       '
       ''>>>>> AQUI  CONTINUAR ...
       '
       '
       '
       'Verifica se existe este campo no portal, se o mesmo existir ele é excluido e reinserido logo abaixo.
       Sql_Portal = Empty
       Sql_Portal = "SELECT " & Campos_gravar_portal & " FROM " & Tabela_portal & " " & _
                    "WHERE " & Campo_comparacao_portal & " = " & strComparacao_retaguarda_portal & ""
       Movimentacoes.Select_geral Sql_Portal, Banco_portal, rstIncluso_portal, Aplicacao_Portal, Form, Usuario, Senha

       'Se existir Excluir
       If Not rstIncluso_portal.BOF = True And Not rstIncluso_portal.EOF = True Then
          funcoes_banco.Excluir Tabela_portal, Campo_comparacao_portal, strComparacao_retaguarda_portal, Aplicacao_Portal, Form, Banco_portal
       End If

       'Montando a string de gravação
       intCont = 0
       strValues = Empty
       Dim itens_value As Long
       
       If Qtd_campos_values_insert_retaguarda = Empty Then
          itens_value = CLng(rstIncluso_portal.Fields.Count)
       Else
          itens_value = CLng(Qtd_campos_values_insert_retaguarda)
       End If

       Do While intCont < itens_value
          If intCont = 0 Then
             If rstReg_nao_atualizados.Fields.Item(intCont).Type = adBoolean Then
                strValues = strValues + CStr(CInt(rstReg_nao_atualizados.Fields(intCont)))
             End If
             If rstReg_nao_atualizados.Fields.Item(intCont).Type = 202 Then
                strValues = strValues + "'" + Funcoes_Gerais.Grava_String(CStr(rstReg_nao_atualizados.Fields(intCont))) + "'"
             End If
             If rstReg_nao_atualizados.Fields.Item(intCont).Type = adTypeText Then
                strValues = strValues + "'" + CStr(rstReg_nao_atualizados.Fields(intCont)) + "'"
             End If
             If rstReg_nao_atualizados.Fields.Item(intCont).Type = 3 Then
                strValues = strValues + CStr(rstReg_nao_atualizados.Fields(intCont))
             End If
             If rstReg_nao_atualizados.Fields.Item(intCont).Type = 135 Then
                strValues = strValues + "'" + CStr(Format(rstReg_nao_atualizados.Fields(intCont), "YYYYMMDD")) + "'"
             End If
          Else
             If rstReg_nao_atualizados.Fields.Item(intCont).Type = adBoolean Then
                strValues = strValues + "," + CStr(CInt(rstReg_nao_atualizados.Fields(intCont)))
       ...
       'Incluir NOVO e Atualizar campo no retaguarda
       funcoes_banco.Gravar_Portal Banco_portal, Tabela_portal, Campos_gravar_portal, strValues, Form, Aplicacao_retaguarda, Banco_retaguarda, Tabela_retaguarda, Campo_Integracao_Retaguarda, Campo_comparacao_retaguarda, strComparacao_retaguarda_portal

       Set rstIncluso_portal = Nothing

       rstReg_nao_atualizados.MoveNext
...
Function GerarChave(valores As Integer)
    '---------------------------------------------------------------------
    'Funçao para criação de senha automática com valores Alfa e Numéricos
    '---------------------------------------------------------------------
    ...
    'Gerando as letras enquanto menor que "Valores"
    Do While Len(Chave) < valores
       num = chr_Array(Int(35 * Rnd))
       Chave = Chave + num
    Loop
    
    'Finalizando gerando a senha
    GerarChave = Chave

End Function
Public Function Gera_Codigo_barra_interno(Codigo_produto As Integer) As String

    Dim strCodigo_barra_interno As String * 7
    Dim intCodigo_produto As Integer
    
    intCodigo_produto = Format(Codigo_produto, "000000")
    
    strCodigo_barra_interno = 4 & intCodigo_produto
    
End Function
Public Function Altera_Resolucao(X As Long, Y As Long, Bits As Long)
...
End Function
Public Function Copiar_Arquivo(strOrigem As String, strDestino As String) As Single
