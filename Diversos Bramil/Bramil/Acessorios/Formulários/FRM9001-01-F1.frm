...
Begin VB.Form FormAcessoSistema 
   ...
   Begin VB.Image Image1 
      Height          =   750
      Left            =   20
      Picture         =   "FRM9001-01-F1.frx":000C
      Top             =   375
      Width           =   2535
   End
...
Sub mpCarregaLocacaodbConfus()
    Dim lrstLocacaoBancoDados As Recordset
'   Set lrstLocacaoBancoDados = New Recordset
    
    CombobCodEsquemaBancoDadostLogin.Clear
    MSFlexGridLocacaoBancoDados.Rows = 1

    Call ppAbre_BDAcesso(pWrkArea, Empty, pdbConfus, Trim(TextstrLocacaobdConfustLogin.Text), pstrSenhaBancoDadosBDCONFUS)
'   Call ppAbre_BDAcesso(pdbConfus, Trim(TextstrLocacaobdConfustLogin.Text), pstrSenhaBancoDadosBDCONFUS)
    ...
    Set lrstLocacaoBancoDados = pfrsSelecao(pdbConfus, pstrSql)
    'pfboQuery pdbConfus, pstrSql, lrstLocacaoBancoDados
        
    If Not lrstLocacaoBancoDados.EOF Then
        CombobCodEsquemaBancoDadostLogin.Text = lrstLocacaoBancoDados("bCodEsquemaBancoDadostLoc")
        pbCodEsquemaBancoDadostLogin = lrstLocacaoBancoDados("bCodEsquemaBancoDadostLoc")
    End If
    While Not lrstLocacaoBancoDados.EOF
        CombobCodEsquemaBancoDadostLogin.AddItem lrstLocacaoBancoDados("bCodEsquemaBancoDadostLoc")
        lrstLocacaoBancoDados.MoveNext
    Wend
    ''''''''''''''''''''
    
    pstrSql = "Select bCodEsquemaBancoDadostLoc, " & _
                     "strCodBancoDadostLoc, " & _
                     "strLocacaoBancoDadostLoc, " & _
                     "bTabTipoBancoDadostLoc " & _
                "from tLocacaoBancoDados " & _
               "WHERE bCodEsquemaBancoDadostLoc = " & _
               Val(CombobCodEsquemaBancoDadostLogin.Text)
    Set lrstLocacaoBancoDados = pfrsSelecao(pdbConfus, pstrSql)
    'pfboQuery pdbConfus, pstrSql, lrstLocacaoBancoDados
    ...
    MSFlexGridLocacaoBancoDados.Rows = 1
    While Not lrstLocacaoBancoDados.EOF
        If InStr(1, pstrBancosUsados, lrstLocacaoBancoDados("strCodBancoDadostLoc"), vbTextCompare) > 0 Then
            MSFlexGridLocacaoBancoDados.Rows = MSFlexGridLocacaoBancoDados.Rows + 1
            MSFlexGridLocacaoBancoDados.TextMatrix(MSFlexGridLocacaoBancoDados.Rows - 1, 0) = lrstLocacaoBancoDados("strCodBancoDadostLoc")
            MSFlexGridLocacaoBancoDados.TextMatrix(MSFlexGridLocacaoBancoDados.Rows - 1, 1) = IIf(lrstLocacaoBancoDados("bTabTipoBancoDadostLoc") = 1, "Access", "SQL")
            MSFlexGridLocacaoBancoDados.TextMatrix(MSFlexGridLocacaoBancoDados.Rows - 1, 2) = lrstLocacaoBancoDados("strLocacaoBancoDadostLoc")
        End If
        lrstLocacaoBancoDados.MoveNext
    Wend
...
Private Sub CombobCodEsquemaBancoDadostLogin_Click()
    ...
    Set lrstLocacaoBancoDados = pfrsSelecao(pdbConfus, pstrSql)
    lrstLocacaoBancoDados.MoveFirst
    MSFlexGridLocacaoBancoDados.Rows = 1
    While Not lrstLocacaoBancoDados.EOF
        If InStr(1, pstrBancosUsados, lrstLocacaoBancoDados("strCodBancoDadostLoc"), vbTextCompare) > 0 Then
            MSFlexGridLocacaoBancoDados.Rows = MSFlexGridLocacaoBancoDados.Rows + 1
...
'    If Dir("C:\InfoMil_Estacao\ESTACAO.TXT") = EMPTY Then
'        MsgBox "Configuração da estação não Encontrada!" & vbCr & "Entre com contato com D.I.", vbCritical, pstrEmpresa
'        pdbConfus.Close
'        Unload Me
'        Exit Sub
'    End If
        
'     Open "C:\InfoMil_Estacao\ESTACAO.TXT" For Input As #1
    
'    Line Input #1, lstrLinha
...
'    If UCase(Left(Trim(lstrLinha), 7)) = "ESTACAO" Then
'        piCodEstacaotLog = Val(Mid(Trim(lstrLinha), 9, 3))
...
'    Close #1
    ...
    lstrLocacaoAcessoriostLogin = prsSeleção("strLocacaoAcessoriostPar")
...
'        Dim lrstLocacaoBancoDados As ADODB.Recordset ''''
...
Private Sub Form_Load()
    ...
    If prstLogin.EOF Then
        MsgBox "Locação do Banco de Dados 'pdbGMUSLOG' não encontrada na Tabela tLocacaoBancoDados", vbCritical + vbOKOnly, "Form_Load"
        pdbGMUSLOG.Close
        End: Exit Sub
    End If
