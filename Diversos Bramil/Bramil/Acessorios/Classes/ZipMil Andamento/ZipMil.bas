...
Public Type DCLIST
  ExtrairSóRecentes  As Long    ' 1 = Extract Only Newer, Else 0
  ...
  PastaExtração      As String  ' The Extraction Directory, NULL If Extracting To Current Dir
End Type

'-- UNZIP32.DLL Userfunctions Structure
Public Type USERFUNCTION
  UZDLLImpressão    As Long     ' Pointer To Apps Print Function
...
Private Type ZPOPT
    ...
    szPastaRz As String ' up to 256 bytes long
End Type

Private Type ZIPUSERFUNCTIONS
    DLLImpressão As Long
    DLLPASSWORD As Long
    DLLCOMMENT As Long
    DLLSERVICE As Long
End Type
...
Function DLLImpressão(ByRef fname As CBChar, ByVal x As Long) As Long
    Dim s0$, xx As Long
...
' Callback for zip32.dll
Function DLLSenha(ByRef s1 As Byte, x As Long, _
                  ByRef s2 As Byte, _
...
Public Function VBZip(argc As Integer, zipname As String, _
          ...
          Optional Encript As Integer = 0, _
    ...
    MYUSER.DLLImpressão = FnPtr(AddressOf DLLImpressão)
    MYUSER.DLLPASSWORD = FnPtr(AddressOf DLLSenha)
...
'    retcode = ZpInit(MYUSER)
    ...
    MYOPT.fEncript = Encript     ' 1 if encryption wanted
    ...
    MYOPT.fUpdate = updat    ' 1 if Atualizar zip file--overwrite only if newer
    ...
    MYOPT.fOffsets = 0       ' 1 if Atualizar archive offsets for sfx Files
    ...
    MYOPT.szPastaRz = UCase$(basename)
...
Public Function UZDLLImpressão(ByRef fname As UNZIPCBChar, ByVal x As Long) As Long
...
Public Function UZDLLSenha(ByRef p As UNZIPCBCh, _
...
Public Function VBUnzip(ByRef sNomeDoZip, ByRef sUnzipDirectory As String, _
                        ByRef iExtrairRecente As Integer, _
                        ByRef iSpaceUnderScore As Integer, _
                        ByRef iProntoRegravar As Integer, _
                        ByRef iQuiet As Integer, _
                        ByRef iWriteStdOut As Integer, _
                        ByRef iTestarZip As Integer, _
                        ByRef iListarExtração As Integer, _
                        ByRef iExtrairSóRecentes As Integer, _
                        ByRef iExibirComentários As Integer, _
                        ByRef iHonorDirectories As Integer, _
                        ByRef iSobregravarArquivos As Integer, _
                        ByRef iConverterCR_CRLF As Integer, _
...
On Error GoTo vbErrorHandler
    ...
    With UZDCL
        .ExtrairSóRecentes = iExtrairSóRecentes
        .SpaceToUnderScore = iSpaceUnderScore
        .PromptToOverwrite = iProntoRegravar
        .fQuiet = iQuiet
        .ncflag = iWriteStdOut
        .ntflag = iTestarZip
        .nvflag = iListarExtração
        .nUflag = iExtrairRecente
        .nzflag = iExibirComentários
        .ndflag = iHonorDirectories
        .noflag = iSobregravarArquivos
        .naflag = iConverterCR_CRLF
        .nZIflag = iVerbose
        .C_flag = iCaseSensitivty
        .fPrivilege = iPrivilege
        .Zip = sNomeDoZip
        .PastaExtração = sUnzipDirectory
    End With
    
    With UZUSER
        .UZDLLImpressão = FnPtr(AddressOf UZDLLImpressão)
        .UZDLLSND = 0&
        .UZDLLREPLACE = FnPtr(AddressOf UZDLLRep)
        .UZDLLPASSWORD = FnPtr(AddressOf UZDLLSenha)
