...
Dim MyDSN, MyTable, MyRetSQL, MySQL, MyIDName, MyIDValue, MyTitolo, MyRitorno, MyRetTitolo, MyRetRitorno, MyRetComandi, MyMainDSN, MyMainSubCommand, MyMainRitorno
...
if MyTitolo="" then
	MyTitolo = "Modify dB Data"
else
	MyTitolo = "Modify " & MyTitolo
end if
...
<form method="POST" action="sql_modifica_cmd.asp" target="_blank">
  ...
  value="<%Response.write(MySQL)%>"><input type="hidden" name="IDName"
  value="<%Response.write(MyIDName)%>"><input type="hidden" name="IDValue"
  value="<%Response.write(MyIDValue)%>"><input type="hidden" name="Titolo"
...
%></p>
  </center></div><div align="center"><center><table BORDER="<%=objRS.Fields.Count%>"
  COLS="2">
    <tr>
