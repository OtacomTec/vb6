...
Dim booFecha_Tela As Boolean
Dim booDesconto_acionado As Boolean
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = 13 And booFecha_Tela = True Then
       Unload Me
    End If
...
Private Sub Form_Load()
    ...
    'Desconto
    Me.Shape9.Visible = False
    Me.Label5.Visible = False
    Me.txtDesconto.Visible = False
    Me.Shape10.Visible = False
    booDesconto_acionado = False
 End Sub
...
Private Sub txtDesconto_LostFocus()
    ...
    If frmTela_Venda.strTipo_desconto = "%" And txtDesconto.Text > 0 Then
       txtTotal_Cupom.Text = Format(CDbl(txtTotal_Cupom) - CDbl(CDbl(txtTotal_Cupom) * CDbl(Me.txtDesconto.Text) / 100), "#,###0.00")
    End If
    
    If frmTela_Venda.strTipo_desconto = "$" Then
       txtTotal_Cupom.Text = Format(CDbl(txtTotal_Cupom) - CDbl(Me.txtDesconto.Text), "#,###0.00")
    End If
...
Public Sub txtValor_pago_LostFocus()
    ...
    If IsNumeric(txtValor_pago.Text) = False Then txtValor_pago.Text = 0
    ...
    If dtcFinalizadora_cupom.Text <> "" Then
        If CDbl(Me.txtValor_pago.Text) > 0 Then
            ...
            Call Fecha_Cupom
        Else
          If booDesconto_acionado = False And frmCliente.Enabled = False Then
             Me.txtValor_pago.SetFocus
          ElseIf booDesconto_acionado = True Then
...
'    End If
    ...
    'Próximo Orçamento
     strSql = Empty
     strSql = "SELECT DFProximo_cupom_TBParametros_ecf," & _
              "DFProximo_serie_cupom_TBParametros_ecf,DFProximo_orcamento_balcao_TBParametros_ecf," & _
              "DFProximo_serie_orcamento_balcao_TBParametros_ecf " & _
              "FROM TBPARAMETROS_ECF WHERE FKCodigo_TBEmpresa = " & frmTela_Venda.strEmpresa_Operador & ""
    ...
    If frmTela_Venda.booVenda_Externa = False Then
    ...
    Else
        lngVendedor = Funcoes_Gerais.Localiza_ID("PKId_TBVendedor", "IXCodigo_TBVendedor", "" & frmTela_Venda.strVendedor_Comanda_Orcamento & "", "TBVendedor", "Otica", Me, "BDRetaguarda")
        lngPlano_pagamento = Funcoes_Gerais.Localiza_ID("PKId_TBPlano_pagamento", "IXCodigo_TBPlano_pagamento", "9999", "TBPlano_pagamento", "Otica", Me, "BDRetaguarda")
    End If
    ...
    'Retornando o tratamento de erro
    On Error GoTo Erro
    
    If frmTela_Venda.booVenda_Externa = True Then
        'Verifica se é cupom abastecido de comanda e finaliza a mesma
        Call Fecha_Venda_Externa
    End If
    ...
    Do While intCont_Itens + 2 < frmTela_Venda.HflexGrid.Rows
        ...
        If frmTela_Venda.booIntegracao_Retaguarda = True Then
            ...
            strSql = "INSERT INTO TBItens_cupom(" & _
                     ...
                     "DFItens_cupom_Registrado_TBItens_cupom,DFControle_bico_TBItens_cupom) "
            strSql = strSql + "VALUES(" & _
                     "" & lngID_Cupom & "," & _
                     "" & strCodigo_item & "," & _
                     "'" & strST & "'," & _
...
Private Function Imprime_Cupom_nao_fiscal()
    ...
    If frmTela_Venda.intTipo_imp_orcamento = 1 Then
       ...
       'Cabeçalho
        ...
        'FINALIZADORAS
        Dim strFinalizadora As String * 20
        Dim strValor As String
        
        intContador = 3
        Do While intContador <= HflexGrid.Rows - 1
           HflexGrid.Row = intContador
           
           HflexGrid.Col = 1
           strFinalizadora = HflexGrid.Text
           HflexGrid.Col = 2
           'Definido com 8 caracteres - para nao distorção do cupom o valor é
           'preenchido com o nº de espaços necessário
           strValor = HflexGrid.Text
           If Len(strValor) < 8 Then
              strValor = Space(8 - Len(strValor)) & strValor
           End If
           
           strLinha_Impressao = strFinalizadora & "                               " & strValor
        ...
        'Definido com 8 caracteres - para nao distorção do cupom o valor é
        'preenchido com o nº de espaços necessário
        strValor = Format(txtTotal_Cupom.Text, "#,###0.00")
        If Len(strValor) < 8 Then
           strValor = Space(8 - Len(strValor)) & strValor
        End If
        
        'TOTAL GERAL
        strLinha_Impressao = "                   Total Geral...:                 " & strValor
        'Mandando o comando direto na porta da impressora
        Funcoes_Gerais.Abre_porta_impressora_via_LPT1 (frmTela_Venda.strCaminho_impComum)
        Print #1, Chr(27) & Chr(15) + strLinha_Impressao 'Imprime caracter Comprimido para matriciais Epson
        Funcoes_Gerais.Fecha_porta_impressora_via_LPT1
        ...
        End If
        'DESCONTO
        strLinha_Impressao = "                   Desconto......:                 " & strValor
        'Mandando o comando direto na porta da impressora
        Funcoes_Gerais.Abre_porta_impressora_via_LPT1 (frmTela_Venda.strCaminho_impComum)
        Print #1, Chr(27) & Chr(15) + strLinha_Impressao 'Imprime caracter Comprimido para matriciais Epson
        Funcoes_Gerais.Fecha_porta_impressora_via_LPT1
        ...
        End If
        'TOTAL A PAGAR
        strLinha_Impressao = "                   Total a Pagar.:                 " & strValor
        'Mandando o comando direto na porta da impressora
        Funcoes_Gerais.Abre_porta_impressora_via_LPT1 (frmTela_Venda.strCaminho_impComum)
        Print #1, Chr(27) & Chr(15) + strLinha_Impressao 'Imprime caracter Comprimido para matriciais Epson
        Funcoes_Gerais.Fecha_porta_impressora_via_LPT1
        
        If txtTroco.Text = Empty Then txtTroco.Text = "0,00"
        
        strValor = Format(CDbl(txtTotal_Cupom.Text) + CDbl(txtTroco.Text), "#,###0.00")
        If Len(strValor) < 8 Then
           strValor = Space(8 - Len(strValor)) & strValor
        End If
        'TOTAL RECEBIDO
        strLinha_Impressao = "                   Total Recebido:                 " & strValor
        'Mandando o comando direto na porta da impressora
        Funcoes_Gerais.Abre_porta_impressora_via_LPT1 (frmTela_Venda.strCaminho_impComum)
        Print #1, Chr(27) & Chr(15) + strLinha_Impressao 'Imprime caracter Comprimido para matriciais Epson
        Funcoes_Gerais.Fecha_porta_impressora_via_LPT1
        
        strValor = Format(txtTroco.Text, "#,###0.00")
        If Len(strValor) < 8 Then
           strValor = Space(8 - Len(strValor)) & strValor
        End If
        'TROCO
        strLinha_Impressao = "                   Troco.........:                 " & strValor
        'Mandando o comando direto na porta da impressora
        Funcoes_Gerais.Abre_porta_impressora_via_LPT1 (frmTela_Venda.strCaminho_impComum)
        Print #1, Chr(27) & Chr(15) + strLinha_Impressao 'Imprime caracter Comprimido para matriciais Epson
        Funcoes_Gerais.Fecha_porta_impressora_via_LPT1
...
Function Fecha_Venda_Externa()

    On Error GoTo Erro
    
    If frmTela_Venda.booComanda_Orcamento = 0 Then
       'Fechando a comnada
       strSql = Empty
       strSql = "UPDATE TBComanda SET DFNumero_cupom_TBComanda = " & strNumero_Nota & " " & _
                "WHERE PKCodigo_TBComanda = " & frmTela_Venda.strNumero_Comanda_Orcamento & ""
    Else
       'Fechando o pedido referente ao orçamento
       strSql = Empty
       strSql = "UPDATE TBPedido SET DFFaturado_TBPedido = 1," & _
                "DFObservacao_TBPedido = LEFT(DFObservacao_TBPedido,110) + '-Faturado pelo Cupom de Nº: " & strNumero_Nota & "' " & _
                "WHERE PKId_TBPedido = " & frmTela_Venda.strNumero_Comanda_Orcamento & ""
    End If
...
Private Function Controle_Transacional_manual()
    ...
    If frmTela_Venda.booIntegracao_Retaguarda = True Then
        ...
        strSql = "DELETE FROM TBTitulo_NOTA_SAIDA WHERE FKId_TBNota_saida = " & lngID_Numero_Nota & ""
        ...
        If lngID_Titulo_receber <> Empty Then
           strSql = Empty
           strSql = "DELETE FROM TBTITULO_Recebido WHERE FKId_TBTitulo_receber = " & lngID_Titulo_receber & ""
