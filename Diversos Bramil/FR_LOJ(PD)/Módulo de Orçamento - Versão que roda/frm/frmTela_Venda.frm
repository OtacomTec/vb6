...
Public strID_Controle_Bomba_bico As Long
...
Public strNome_impressora As String
...
Public booVenda_Externa As Boolean
Public strNumero_Comanda_Orcamento As String
Public strVendedor_Comanda_Orcamento As String
Public booComanda_Orcamento As Boolean
Public intNivel As Integer
Public intNivel_Operador As Integer
'MOEDA
Public booMult_Moeda As Boolean
Public intCodigo_Moeda_Corrente As Integer
Dim rstCotacao_Moeda As New ADODB.Recordset
Option Explicit
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 113 Then
       ...
       'Setando os paradores como false para cliques de aberturas de telas
    ...
    If KeyCode = 116 And Me.intNivel > 1 Then
       booAcionado_Fechamento_cupom = True
       ...
       If Me.HflexGrid.Rows - 3 > 0 Then
        ...
        txtCodigo_Produto.SetFocus
        Me.txtStatus.Text = "Aguardando Venda...."
        'Setando Nivel do usuário para o nível original
        Me.intNivel = Me.intNivel_Operador
    End If
    
    'Verifica se foi precionado F6 e Cancela o cupom
    If KeyCode = 117 And Me.intNivel > 1 Then
       booAcionado_Fechamento_cupom = True
    ...
    If KeyCode = 119 And Me.intNivel > 1 Then
       'Setando os paradores como false para cliques de aberturas de telas
       Me.txtPreco_Unitario.TabStop = False
       txtQuantidade_Produto.TabStop = False
    ...
    If KeyCode = 121 And Me.intNivel > 1 Then
...
Private Sub Form_KeyPress(KeyAscii As Integer)
    ...
    'Se perfil for de posto e acionado o CTRL + E abre a opção de venda por encerrante
    ...
    If KeyAscii = 24 Then
       If Me.intTipo_imp_orcamento <> 1 Then
    ...
    'Configurações da impressora - CTRL + B
    If KeyAscii = 2 Then
       If Me.intTipo_imp_orcamento <> 1 Then
    ...
    'Configurações da impressora - CTRL + L
    If KeyAscii = 12 Then
       booConsulta = True
    ...
    'Verifica se foi precionado CTRL+O e abre a tela de orçamento
    If KeyAscii = 15 Then
       'Setando os paradores como false para cliques de aberturas de telas
       Me.txtPreco_Unitario.TabStop = False
       txtQuantidade_Produto.TabStop = False
...
Private Sub Form_Load()
    ...
    'Tratando quando não houver imagem gravada
    ...
    If txtCodigo_Produto.Text <> "" Then
        ...
        If Len(Me.txtCodigo_Produto.Text) < 7 Then
           If booIntegracao_Retaguarda = True And booPreco_online = True Then
              ...
              strSql = Empty
              strSql = "SELECT TBProduto.PKId_TBProduto,TBProduto.DFCst1_TBProduto,TBProduto.DFCst2_TBProduto," & _
                       "TBProduto.IXCodigo_TBProduto,TBProduto.DFDescricao_resumida_TBProduto," & _
                       "TBITENS_TABELA_PRECO.DFPreco_varejo_TBItens_tabela_preco,DFPath_imagem_TBProduto," & _
                       "FKCodigo_TBMoeda " & _
                       "FROM TBProduto " & _
              ...
              If frmTela_Venda.booEncerrante = False Then
                 'VERIFICA A MOEDA REFERENTE AO ITEM NA TABELA
                 If Me.booMult_Moeda = False Then
                    txtPreco_Unitario.Text = rstInf_Produtos!DFPreco_varejo_TBItens_tabela_preco
                 Else
                    'VERIFICA SE O ITEM ESTÁ DEFINIDO COM A MOEDA CORRENTE
                    If rstInf_Produtos!FKCodigo_TBMoeda = intCodigo_Moeda_Corrente Then
                       txtPreco_Unitario.Text = rstInf_Produtos!DFPreco_varejo_TBItens_tabela_preco
                    Else
                       strSql = "SELECT DFData_TBCotacao_moeda,DFValor_TBCotacao_moeda FROM TBCotacao_Moeda " & _
                                "WHERE FKCodigo_TBMoeda = " & rstInf_Produtos!FKCodigo_TBMoeda & " " & _
                                "AND DFData_TBCotacao_moeda = (SELECT MAX(DFData_TBCotacao_moeda) FROM TBCotacao_moeda)"
                       
                       Select_geral strSql, "BDRetaguarda", rstCotacao_Moeda, "Otica", Me
                       
                       If rstCotacao_Moeda.RecordCount <> 0 And IsNull(rstCotacao_Moeda.Fields("DFValor_TBCotacao_moeda")) = False Then
                          txtPreco_Unitario.Text = rstInf_Produtos!DFPreco_varejo_TBItens_tabela_preco * rstCotacao_Moeda.Fields("DFValor_TBCotacao_moeda")
                       Else
                          txtPreco_Unitario.Text = rstInf_Produtos!DFPreco_varejo_TBItens_tabela_preco
                       End If
                       Set rstCotacao_Moeda = Nothing
                    End If
                 End If
              End If
            ...
            Else
              ...
              strSql = "SELECT TBProduto.PKId_TBProduto,TBProduto.DFCst1_TBProduto,TBProduto.DFCst2_TBProduto," & _
                       "TBProduto.IXCodigo_TBProduto,TBProduto.DFDescricao_resumida_TBProduto," & _
                       "TBProduto.DFPreco_venda_TBProduto,TBProduto.DFPath_imagem_TBProduto " & _
                       "FROM TBProduto " & _
        ...
        If Len(Me.txtCodigo_Produto.Text) > 6 Then
            ...
            If strDigito_Peso_Variavel = strDigito_Produto_Digitado Then
                ...
                If booIntegracao_Retaguarda = True And booPreco_online = True Then
                   ...
                   strSql = "SELECT TBProduto.PKId_TBProduto,TBProduto.DFCst1_TBProduto,TBProduto.DFCst2_TBProduto," & _
                            ...
                            "DFPath_imagem_TBProduto,FKCodigo_TBMoeda " & _
                            "FROM TBProduto " & _
                            "INNER JOIN TBITENS_TABELA_PRECO " & _
                            "ON TBPRODUTO.PKID_TBProduto = TBITENS_TABELA_PRECO.FKID_TBProduto " & _
                            "WHERE TBITENS_TABELA_PRECO.FKCodigo_TBTabela_preco = (SELECT DFNumero_tabela_vigente_TBParametros_venda FROM TBPARAMETROS_VENDA WHERE IXCodigo_TBEmpresa = " & strEmpresa_Operador & ") " & _
            ...
            Else
                ...
                If booIntegracao_Retaguarda = True And booPreco_online = True Then
                    ...
                    If rstInf_Produtos.BOF = True And rstInf_Produtos.EOF = True Then
                       MsgBox "Código de barra não cadastrado ou preço na tabela vigente não cadastrado.Verifique!", vbCritical, "Only Tech"
                       txtCodigo_Produto.Text = Empty
                       txtCodigo_Produto.SetFocus
                       Me.txtPreco_Unitario.TabStop = False
                       Me.txtQuantidade_Produto.TabStop = False
...
Private Function Processando_item()
     ...
     HflexGrid.Col = 3
     HflexGrid.Text = Trim(strQuantiade)
     HflexGrid.Col = 4
     HflexGrid.Text = "X"
     HflexGrid.Col = 5
...
Private Function carrega_imagem()
    ...
    If IsNull(rstInf_Produtos!DFPath_imagem_TBProduto) = False Then
...
Private Sub txtQuantidade_Produto_LostFocus()
    If txtQuantidade_Produto.Text = Empty Then txtQuantidade_Produto.Text = "0"
    If Me.txtQuantidade_Produto.TabStop = True Then
       ...
       If txtQuantidade_Produto.Text = 0 Then
          txtQuantidade_Produto.Text = Empty
          'PARA CORREÇÃO DO PROBLEMA DE LOOP DE FOCU
          '- CASO O PRODUTO NAO TENHA SIDO INFORMADO O FOCU É ENVIADO PARA ELE
...
Private Function Verifica_ICMS()
    ...
    Else
        ...
        Else
           If IsNull(rstVerifica_Estado_ICMS!DFPercentual_icms_saida_fisica_TBEstado_icms) Then
              intAliquota_ICMS = 0
           Else
...
Private Function Cancela_cupom()
    ...
    'Verifica se existe cupom não gravado
    'Pois se esta recordset estiver realmente vazia, representa q ele terá q ir cancelar o ultimo gravado no banco
    'Caso contrário...limpará somente a tela e eliminará o mesmo da tabela e nem entrará no IF abaixo
    '                                              ||
    '                                              ||
    '                                              ||
    '                                              ||
    ...
    If rstCupom_processado.BOF = True And rstCupom_processado.EOF = True Then
        ...
        If booIntegracao_Retaguarda = True Then
            ...
            '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            'LENDO A TABELA DE ITENS NOTA E BUSCANDO A QUANTIDADE QUE SERÁ ADICIONADA '
            'AO ESTOQUE.                                                              '
            '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            ...
            Do While rstBusca_Quantidade.EOF = False
                ...
                ocorrencia.Observacao = "Canc. N.F.Saída Nº:" & Nota & "-" & Serie & "-Adição de Estoque"
                ocorrencia.Programa = "Canc.N.F.Saída - PDV"
